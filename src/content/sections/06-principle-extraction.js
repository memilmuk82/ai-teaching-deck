...
export const slides = [
  {
    id: '26', sectionId: 'principle-extraction', sectionTitle: 'AI와 수업 원칙 추출', title: '이제 AI의 질문에 답하며 나의 수업 원칙을 만듭니다', layout: 'comparison', durationSeconds: 120,
    toolContext: { kind: 'gemini', label: '일반 Gemini', detail: '새 대화 준비' },
    character: [{ role: 'teacher', position: 'left' }, { role: 'ai-helper', position: 'right' }],
    blocks: [
      { type: 'callout', label: '지금 할 일', text: 'AI를 열고 27번 프롬프트를 복사할 준비를 해 주세요.', tone: 'blue' },
      { type: 'comparison', left: { title: '교사가 할 일', blocks: [{ type: 'paragraph', text: '학생과 수업에 관한 실제 경험을 답합니다.' }, { type: 'bullets', items: ['학생이 어디에서 어려워하는가', '어떤 활동이 필요한가', '무엇을 피해야 하는가', '어떤 지원과 성공 기준이 필요한가'] }] }, right: { title: 'AI가 할 일', blocks: [{ type: 'paragraph', text: '답변을 고정값, 변수, 확인 가능한 규칙으로 정리합니다.' }, { type: 'bullets', items: ['한 번에 최대 2개 질문', '선택지가 필요한 질문 제시', '모호한 답을 확인 가능한 규칙으로 변환', '고정값과 변수 구분', 'Gem 지시사항 초안 작성'] }] } },
      { type: 'headline', text: '선생님은 문장이 아니라 수업을 결정합니다', tone: 'blue' },
      { type: 'process', items: ['답변', '규칙 후보', '교사 확인'] },
      {
        type: 'columns',
        columns: [
          { title: '지금은 하지 않을 일', blocks: [{ type: 'callout', text: '실제 수업자료 제작\n이 단계에서는 실제 수업자료를 만들지 않습니다.', tone: 'yellow' }] },
          { title: '결과물', blocks: [{ type: 'callout', text: '나의 수업 원칙 초안', tone: 'green' }] },
        ],
      },
      { type: 'callout', label: '다음 단계', text: '27번 슬라이드의 프롬프트를 복사해 AI와 대화를 시작합니다.', tone: 'blue' },
    ],
    notes: ['참여자에게 클래식 Gem이 아닌 일반 Gemini의 새 대화를 열고 27번의 복사 버튼을 누를 준비를 하게 합니다. 교사가 프롬프트 전문가가 될 필요는 없습니다. 학생과 수업에 대한 판단은 교사가 하고, AI는 그 판단을 일관된 문장으로 정리하게 합니다.', '참여자는 “학생이 긴 지시문을 어려워한다”, “토론 전에 개인 판단이 필요하다”처럼 실제 경험을 답할 준비를 합니다. AI가 제시한 규칙은 초안이며 교사가 확인하고 수정해야 합니다.', '설계 대화가 끝날 때까지 자료를 만들지 않게 해야 대화의 초점이 흐려지지 않습니다. “이제 일반 Gemini 새 대화에 27번 프롬프트를 복사해 첫 두 질문에 답하겠습니다”라고 안내하며 다음 슬라이드로 넘어갑니다.'],
  },
  {
    ...
    toolContext: { kind: 'gemini', label: '일반 Gemini', detail: '새 대화에서 실행' },
    character: { role: 'ai-helper', position: 'right' },
    blocks: [
      { type: 'paragraph', text: '프롬프트를 일반 Gemini 새 대화에 복사한 뒤, AI가 묻는 질문에 자신의 실제 수업 경험으로 답합니다.' },
  ...
  },
  {
    ...
    toolContext: { kind: 'gemini', label: '일반 Gemini', detail: '같은 설계 대화' },
  ...
  },
  {
    ...
    toolContext: { kind: 'gemini', label: '일반 Gemini', detail: '규칙 초안 확인' },
